...
- **Issue key**: 그 고객에게 묶인 API 키를 발급한다. 키의 원문은 **딱 한 번** 보여주므로 그 자리에서 복사해 고객에게 전달한다. 이 키가 곧 그 고객의 크리덴셜이다.
- 발급된 키가 enqueue 하는 모든 잡·플로우런에는 **고객 정체성이 각인**되어 사용량·과금·감사·읽기 격리의 기반이 된다.
...
액션에 넘길 **입력 JSON**을 계층으로 설정한다. 앱 상세의 **Config** 탭에서 연다.
