...
고객 하나의 세 가지 — **키**(어떻게 인증하나) · **접근·설정**(뭘 호출할 수 있고 어떤 입력으로) · **잡**(그 고객의 실행) — 을 한 화면에 모은다. 런타임엔 하나(키 → 고객 각인 → 입력 오버레이 → 자기 잡 읽기)인 것을, 관리도 한곳에서 한다.

![고객 상세](../images/ui-alpha/62-customer-detail.png)

- **Keys**: 그 고객의 API 키 목록(label·prefix·마지막 사용·상태)과 **Issue key**·키별 **Revoke**. 키 발급은 **label 과 만료(선택)만** 받는다 — 고객 키에 scope 는 없다. 무엇을 할 수 있는지는 아래 **Access & config** 의 grant 가 정한다. 키의 원문은 **딱 한 번** 보여주므로 그 자리서 복사해 고객에게 전달한다.
- **Access & config**: 그 고객이 grant 받은 `(app, action)` 을 앱 넘어 한눈에. **Grant an action** 으로 앱·액션을 고르면 그 자리서 입력 설정을 편집한다 — **grant 를 만드는 것이 곧 그 액션에 대한 인가**다(빈 설정으로 저장하면 오버라이드 없이 접근만 부여). 키는 있는데 grant 가 없으면 "키가 아직 무력"이라는 경고가 뜬다.
- **Usage**: 그 고객의 기간별 미터링 사용량(실행 수·런타임, outcome 별) — 기간 프리셋(이번 달·지난달·최근 30일)과 **Export CSV**. 이 수치는 잡 행이 아니라 **불변 과금 원장**(usage_event)에서 계산되므로, 잡이 retention 으로 지워진 뒤에도 정확하다. CSV 는 항목 원장(시각·잡 id·앱·액션·outcome·런타임) 그대로라 **과금 분쟁의 증거**로 쓴다 — 페이로드는 원장에 아예 없어 새지 않는다.
- **Recent jobs**: 그 고객의 최근 잡만(디버그 렌즈 — retention 창 안).
